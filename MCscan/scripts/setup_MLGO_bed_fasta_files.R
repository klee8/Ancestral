...
###########################################
#   Create MLGO 'bed' file for MCscan     #
###########################################
...
# merge Eam702 gene positions from gff to df 
...
head(df)
# write Eama fasta sequence faidx lookup file
#df$faidx <- gsub("Eam", ">Eam", df$V1)
df$faidx <- paste(df$V1, ":", df$V4)
df$faidx <- paste(df$faidx, "-",  df$V5)
df$faidx <- gsub(" ", "", df$faidx)

write.table(df$faidx, "data/MLGO_A6_Eama_regions.txt", quote = FALSE, row.names = FALSE)

# write fasta header file
df$header <- gsub("mRNA", ">mRNA", df$name)
df$header <- paste(df$header, df$faidx)

write.table(df$header, "data/MLGO_A6_fasta_headers.txt", quote = FALSE, row.names = FALSE)

# subset to bedfile
bedfile <- df[, c("gen_chr", "start", "end", "name", "score", "strand")]

# write bedfile
write.table(bedfile, "data/MLGO_A6.bed", quote = FALSE, row.names = FALSE, sep="\t")
